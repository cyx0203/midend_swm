...
import { eq, equals, gt, isArray, join, lt, map, pick, pipe, prop, T } from 'lodash/fp';
import {
  fetchChargeRecord,
  fetchFeeDetail,
  fetchFeeMaster,
...
import { FeeDetailColumns, FeeMasterColumns, _CashColumns, _ChargeColumns } from '../vtm/columns';
...
export default function CheckForm() {
  ...
  const { run: runRecordDetail } = useRequest(fetchRecordDetail, {
    ...
    onSuccess: res => {
      if (isMOPSucceed(res.returnresult.returncode)) {
        if (isArray(res.data.data_row)) {
          const [node] = res.data.data_row;
          node.needpay = +node.needpay;
          setRecordDetail(node);
        }

        setRecordDetail(res.data.data_row as unknown as MOP.PayDetail);
      }
    },
  });
  ...
  const handleUserQuery = async fields => {
    ...
    const res = await fetchUserRecords(requestParams);
    if (isMOPSucceed(res.returnresult.returncode)) {
      // setDescription(res.data.data_row as unknown as MOP.UserRecord);
      if (isArray(res.data.data_row)) {
        return {
          data: res.data.data_row,
        };
      } else {
        return {
          data: [res.data.data_row],
  ...
  const handleCashQuery = async ({ jslsh }) => {
    ...
    const res = await fetchFees(requestParams);
    if (isMOPSucceed(res.returnresult.returncode)) {
      return {
        data: res.data.data_row,
      };
  ...
  const handleChargeQuery = async ({ jslsh }) => {
    ...
    const res = await fetchChargeRecord(requestParams);
    if (isMOPSucceed(res.returnresult.returncode)) {
      setRefoundTable(res.data.data_row.map(v => ({ ...v, refound: 0 })));
      return {
        data: res.data.data_row,
      };
    }
  ...
  const handleFeeMasterQuery = async () => {
    ...
    const res = await fetchFeeMaster(requestParams);
    if (isMOPSucceed(res.returnresult.returncode)) {
      return {
        data: res.data.data_row,
      };
  ...
  const handleFeeDetailQuery = async () => {
    ...
    const res = await fetchFeeDetail(requestParams);
    if (isMOPSucceed(res.returnresult.returncode)) {
      return {
        data: res.data.data_row,
      };
    ...
    <aside className={clsx(styles.container, { [styles.expanded]: expand })}>
      ...
      >
        <ProForm<FormType>
          ...
          initialValues={{ cardType: methods[0], cardNo: '219935266', location: fyMapper[0].value }}
          onFinish={async () => {
            openUserModal();
          }}
          form={InfoForm}
              ...
              render: (_, record) => (
                <a
                  ...
                  onClick={() => {
                    ...
                    runRecordDetail(prepareRequestParams('2'));
                    closeUserModal();
                  }}
                >
                  选择
