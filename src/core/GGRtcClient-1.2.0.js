...
export default class GGRtcClient {
		...
		this._peer.onconnectionstatechange = (event) => {
			...
			switch (this._peer.connectionState) {
				...
				case 'disconnected':
						...
						if (JSON.stringify(callbackFunc) !== '{}'&&callbackFunc[cbFuncName2]) {
							callbackFunc[cbFuncName2]();
						}
				...
				case 'closed':
						...
						if (callbackFunc[cbFuncName3]) {
							callbackFunc[cbFuncName3]();
						}
					break;
  ...
  // 停止录制 - 远端
  _stopRemoteRecord() {
    if (!this._mediaRemoteRecorder || this._mediaLocalRecorder.state === 'inactive') return;
  ...
  _stopRemoteShareRecord() {
    if (!this._mediaRemoteShareRecorder || this._mediaLocalRecorder.state === 'inactive') return;
