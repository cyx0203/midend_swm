...
const DomRoot = (props: any) => {
    ...
    const needKeepAlive: boolean = KEEPALIVE;
    ...
    // console.log(`该页面是否要KA：${needKeepAlive}`);

    //不需要keepalive的根节点DOM返回内容
    if (!needKeepAlive) {
        return <PageContainer loading={props.loading} className={styles.root}>{props.children}</PageContainer>
    }
    //需要keepalive的根节点DOM返回内容
    return <KeepAlive persistant={location.pathname === '/welcome' ? true : false}>
        <PageContainer
            // 不需要头部区域
...
export { DomRoot, HttpReqPost, HttpReqGet, Ajax };